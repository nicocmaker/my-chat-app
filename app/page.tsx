...
export default function Home() {
  ...
  if (!user && page !== "auth") {
    return <div style={{ textAlign: "center", paddingTop: "100px" }}><button onClick={() => setPage("auth")} style={{ padding: "10px 20px" }}>ログイン / 新規登録</button></div>;
  }
      ...
      <main style={{ width: "100%", maxWidth: "500px", margin: "0 auto", background: "#fff", minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative" }}>
        ...
        {(page === "global" || page === "dm") && (
          <div style={{ position: "fixed", bottom: "65px", width: "100%", maxWidth: "500px", background: "#fff", borderTop: "1px solid #eee", padding: "10px", zIndex: 100 }}>
            <div style={{ display: "flex", gap: "10px" }}>
              <textarea value={text} onChange={e => setText(e.target.value)} placeholder="入力..." style={{ flex: 1, border: "none", outline: "none", resize: "none" }} />
              <button onClick={() => sendPost(page === "dm")} style={{ ...btnStyle, background: "#000", color: "#fff", padding: "5px 20px", borderRadius: "20px" }}>送信</button>
            </div>
          </div>
        )}
        ...
        <nav style={{ display: "flex", position: "fixed", bottom: 0, width: "100%", maxWidth: "500px", borderTop: "1px solid #eee", background: "#fff" }}>
          ...
          <button onClick={() => setPage("friends")} style={{ ...btnStyle, flex: 1, padding: "15px", color: page === "friends" ? "#000" : "#ccc" }}>👥</button>
